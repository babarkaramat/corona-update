...
export class DetailsComponent implements OnInit {
  ...
  public primaryYAxisdeath: Object = {
    rangePadding: "None",
    interval: 500,
    lineStyle: { width: 0 },
    majorTickLines: { width: 0 },
  ...
  ngOnInit() {
    ...
    if (this.type.type == 'cases' && this.direct == 1) {
      this.service.getCases().pipe(map(x => { return x.body })).subscribe((res: any) => {
        this.cases = res;
      })
    }

    if (this.type.type == 'death' && this.direct == 1) {
      this.service.getDeath().pipe(map(x => { return x.body })).subscribe((res: any) => {
        this.death = res;
      })
    }
    if (this.type.type == 'admit' && this.direct == 1) {
      this.service.getAdmit().pipe(map(x => { return x.body })).subscribe((res: any) => {
        this.admit = res.filter(x => x.newAdmissions != null)
      })
    }
    if (this.type.type == 'test' && this.direct == 1) {
      this.service.getTest().pipe(map(x => { return x.body })).subscribe((res: any) => {
